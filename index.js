...
const MAIN_PLATFORM_URL = process.env.MAIN_PLATFORM_URL   // https://zynq-platform.up.railway.app
const MAIN_PLATFORM_SECRET = process.env.MAIN_PLATFORM_SECRET  // same as NUXT_ADMIN_SECRET

// ─── Questions ────────────────────────────────────────────────────────────────
const QUESTIONS = [
  {
    field: 'noticePeriod',
    text: 'What is your current notice period? You can just say the number of days.',
  },
  {
    field: 'currentCtc',
    text: 'What is your current C T C — your annual salary in lakhs?',
  },
  {
    field: 'expectedCtc',
    text: 'And what is your expected C T C — the salary you are targeting for your next role?',
  },
  {
    field: 'activelyLooking',
    text: 'Last question. Are you actively looking for a new job right now, or just exploring options?',
  },
]

const ACKS = ['Got it.', 'Perfect, thank you.', 'Understood.', 'Great.']

// ─── In-memory sessions (per WebSocket connection) ────────────────────────────
const sessions = new Map()

// ─── HTTP server (Plivo webhooks) ─────────────────────────────────────────────
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && url.pathname === '/answer') {
    ...
    req.on('end', () => {
      // Params from Plivo come as query string on answer URL
      ...
      const agentBaseUrl = process.env.AGENT_URL || `http://localhost:${PORT}`
      const wsUrl = agentBaseUrl.replace('https://', 'wss://').replace('http://', 'ws://') + '/stream'

      const xml = `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Stream wsUrl="${wsUrl}" bidirectional="true">
    <Parameter name="candidateId" value="${candidateId}" />
    <Parameter name="candidateName" value="${encodeURIComponent(candidateName)}" />
    <Parameter name="jobTitle" value="${encodeURIComponent(jobTitle)}" />
    <Parameter name="agencyName" value="${encodeURIComponent(agencyName)}" />
  </Stream>
</Response>`

      res.writeHead(200, { 'Content-Type': 'application/xml' })
      res.end(xml)
      console.log(`[HTTP] /answer → streaming call for candidate ${candidateId}`)
    })
    return
  }
...
async function onSilence(ws, session) {
  if (session.audioChunks.length < 8) {
    // Too little audio — background noise, keep listening
    session.audioChunks = []
    return
  }
  ...
  if (q) {
    session.answers[q.field] = transcript || 'no_response'
  }
  ...
  if (nextIndex >= QUESTIONS.length) {
    // All done — thank candidate, save answers, hang up
    ...
    // Save all answers to main platform
...
async function speak(ws, text) {
  if (ws.readyState !== 1) return  // ws.OPEN = 1
  try {
    ...
    // Small pause so Plivo finishes playing before we start listening
...
async function saveAnswers(session) {
  if (!MAIN_PLATFORM_URL || !session.candidateId) {
    console.log('[WS] Skipping save — no platform URL or candidateId')
    console.log('[WS] Answers:', session.answers)
    return
  }
  try {
    const response = await fetch(`${MAIN_PLATFORM_URL}/api/voice/results`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        candidateId: session.candidateId,
        secret: MAIN_PLATFORM_SECRET,
        ...session.answers,
      }),
    })
    if (response.ok) {
      console.log(`[WS] Answers saved for candidate ${session.candidateId}`)
    } else {
      console.error('[WS] Save failed:', response.status, await response.text())
    }
  } catch (err) {
...
// ─── mulaw → WAV conversion (pure JS, no deps) ───────────────────────────────
